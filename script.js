...
form.addEventListener('submit', event => { event.preventDefault(); if (!form.reportValidity()) return; const data = new FormData(form); const body = `Hello Lotus Villa,\n\nI would like to enquire about a stay.\n\nName: ${data.get('name')}\nEmail: ${data.get('email')}\nCheck-in: ${data.get('checkin')}\nCheck-out: ${data.get('checkout')}\nGuests: ${data.get('guests')}\n\n${data.get('message') || ''}\n\nPlease share availability and rates. Thank you.`; window.location.href = `mailto:reservations.lotusvilla@gmail.com?subject=${encodeURIComponent('Lotus Villa — Reservation Enquiry')}&body=${encodeURIComponent(body)}`; document.querySelector('#booking-status').textContent = 'Your email app will open with your enquiry. Please send it there. If no email app opens, call +91 9953555771. Your booking is confirmed only after our team responds.'; });
